...
public class SubmitFXMLController implements Initializable {
    ...
    @FXML
    private void SubmitButtonClicked(ActionEvent event) throws IOException, ClassNotFoundException {
        ...
        if (!ProblemIdMessage.isVisible() || ProblemSet.Problems.containsKey(ProblemName.getText())) {
            ...
            //SubmissionHistory submissionHistory = (SubmissionHistory) LocalUser.getOis().readObject();
            //Submission sm= (Submission) LocalUser.getOis().readObject();
            //System.out.println(sm.getVerdict());
